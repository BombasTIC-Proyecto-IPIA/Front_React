...
const MenuPacientes = (props) => {
  ...
  useEffect(() => {
    const fetchDataPaciente = async () => {
      ...
      try {
        const responsePaciente = await fetch(`http://172.20.10.2:3000/api/paciente/d/${user.dni}`);
        const dataPaciente = await responsePaciente.json();

        const responseDiagnosticos = await fetch('http://172.20.10.2:3000/api/diagnostico');
